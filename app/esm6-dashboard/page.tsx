...
interface ESM6Data {
  success: boolean;
  error?: string;
  es:  { price: number; change: number; changePct: number; high?: number; low?: number } | null;
  nq:  { price: number; change: number; changePct: number } | null;
  vix: { price: number; change: number; changePct: number } | null;
  spy: { price: number; change: number; changePct: number } | null;
  esPrice: number;
  contractNote: string;
  orb: {
  ...
  internals: {
    ...
    aapl: { price: number; changePct: number; interpretation: string } | null;
  } | null;
  risk: {
    stopPts: number; t1Pts: number; t2Pts: number; t3Pts: number;
    stopDir: number; t1: number; t2: number; t3: number;
    mesPerStop: number; rr1: string; rr2: string;
  };
...
function ESM6Chart({ data, orb, vwap, ema9, ema21, liquidity }: {
  data: { time: number; open: number; high: number; low: number; close: number; volume: number }[];
  orb: ESM6Data['orb'];
  vwap: number;
  ema9: number | null;
  ema21: number | null;
  liquidity: ESM6Data['liquidity'];
}) {
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <ComposedChart data={chartData} margin={{ top: 5, right: 60, bottom: 5, left: 10 }}>
        ...
        {/* ORB zone */}
        <ReferenceArea yAxisId="price" y1={orb.low} y2={orb.high} fill="#1d4ed8" fillOpacity={0.05} />
        ...
        {/* Price area */}
        <Area yAxisId="price" type="monotone" dataKey="close" stroke="#3b82f6" fill="#3b82f615"
          strokeWidth={2} dot={false} activeDot={{ r: 3, fill: '#60a5fa' }} />
  ...
  if (!htf) {
    return (
      <DarkCard accent="border-l-gray-700">
        <SectionTitle icon={<BarChart2 size={15} />} title="HTF Alignment"
          right={<button onClick={load} className="text-xs text-blue-400 hover:text-blue-300">Load</button>} />
...
export default function ESM6Dashboard() {
  const [timeframe, setTimeframe] = useState<TF>(5);
  const [data, setData]           = useState<ESM6Data | null>(null);
  const [loading, setLoading]     = useState(false);
  ...
  const [session, setSession] = useState<SessionState>(() => {
    ...
    const saved = localStorage.getItem('esm6-session-v1');
    if (saved) {
      try {
  ...
  const saveSession = useCallback((s: SessionState) => {
    setSession(s);
    localStorage.setItem('esm6-session-v1', JSON.stringify({ ...s, date: new Date().toDateString() }));
  }, []);
  ...
  const doFetch = useCallback(async () => {
    ...
    setError('');
    try {
      const res  = await window.fetch(`/api/esm6?timeframe=${timeframe}`);
      const json: ESM6Data = await res.json();
  ...
  return (
    <AppShell title="ESM6 / ES Futures Dashboard">
      <div className="-m-4 lg:-m-6 bg-gray-950 min-h-screen p-4 lg:p-5">
        ...
        {!data && !loading && !error && (
          <div className="text-center py-24">
            <BarChart2 size={52} className="text-blue-800 mx-auto mb-4" />
            <p className="text-gray-300 font-semibold text-lg">Select a timeframe and click Analyze</p>
            <p className="text-gray-600 text-sm mt-2">Fetches live ES futures data · bias engine · ORB · session · liquidity</p>
          </div>
        )}
        ...
        {loading && !data && (
          <div className="text-center py-24">
            ...
            <p className="text-gray-500 text-sm mt-4">Fetching ES futures data…</p>
          </div>
        )}
        ...
        {data && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {data.es && (
                <div className="flex flex-col items-center px-4 py-2 rounded-xl border bg-blue-900/20 border-blue-700">
                  <span className="text-xs text-gray-400 font-medium">ES / MES</span>
                  <span className="text-xl font-black text-gray-100">{data.es.price.toLocaleString()}</span>
                  <span className={`text-xs font-semibold ${pctColor(data.es.changePct)}`}>{fmt(data.es.changePct, 2)}%</span>
                </div>
              )}
              {[
                { label: 'NQ', q: data.nq },
                { label: 'VIX', q: data.vix },
                { label: 'SPY', q: data.spy },
              ].map(({ label, q }) => q ? (
              ...
              {data.internals?.aapl && (
                <div className="flex flex-col items-center px-3 py-2 rounded-xl border bg-gray-900 border-gray-800">
                  <span className="text-xs text-gray-400">AAPL</span>
                  <span className="text-base font-bold text-gray-100">{data.internals.aapl.price.toFixed(2)}</span>
                  <span className={`text-xs font-semibold ${pctColor(data.internals.aapl.changePct)}`}>{fmt(data.internals.aapl.changePct, 2)}%</span>
                </div>
              )}
            </div>
              ...
              <DarkCard>
                ...
                <div className="grid grid-cols-2 gap-1.5 text-xs">
                  <div className="bg-gray-800 rounded-lg p-2">
                    ...
                    <p className={`font-bold ${data.esPrice >= data.vwap ? 'text-emerald-400' : 'text-red-400'}`}>
                      {data.vwap.toFixed(0)} {data.esPrice >= data.vwap ? '↑' : '↓'}
                    </p>
                  </div>
                  <div className="bg-gray-800 rounded-lg p-2">
                  ...
                  <div className="bg-gray-800 rounded-lg p-2">
                    ...
                    <p className={`font-bold ${data.atr >= 15 ? 'text-emerald-400' : 'text-amber-400'}`}>{data.atr.toFixed(1)} pts</p>
                  </div>
                  <div className="bg-gray-800 rounded-lg p-2">
            ...
            <DarkCard>
              <SectionTitle
                ...
                title={`ES Futures — ${data.orb.timeframe}m Chart`}
                right={<span className="text-xs text-gray-600">{data.fetchedAt.slice(11, 19)} UTC</span>}
              />
              <div className="flex flex-wrap gap-4 text-xs text-gray-500 mb-2">
                <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-blue-400 inline-block" /> Price</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <DarkCard>
                ...
                <div className="space-y-1 text-xs mb-4">
                  {[
                    { label: 'T3 Up 2.0×', price: data.orb.t3Up, color: 'text-emerald-300', active: data.esPrice >= data.orb.t3Up },
                    { label: 'T2 Up 1.0×', price: data.orb.t2Up, color: 'text-emerald-400', active: data.esPrice >= data.orb.t2Up },
                    { label: 'T1 Up 0.5×', price: data.orb.t1Up, color: 'text-emerald-500', active: data.esPrice >= data.orb.t1Up },
                    { label: 'ORB HIGH',   price: data.orb.high, color: 'text-emerald-400 font-bold', active: data.esPrice >= data.orb.high, border: true },
                  ].map(row => (
                    <div key={row.label} className={`flex justify-between px-2 py-1 rounded ${row.active ? 'bg-emerald-950/40' : ''} ${row.border ? 'border-b border-gray-800 pb-2 mb-1' : ''}`}>
                      ...
                      <span className="text-gray-500">{row.active ? '✓' : `+${(row.price - data.esPrice).toFixed(0)}pt`}</span>
                    </div>
                  ))}
                  <div className="flex justify-between px-2 py-1.5 rounded bg-blue-900/30 border border-blue-800">
                    <span className="text-blue-300 font-bold">CURRENT</span>
                    <span className="text-blue-200 font-bold">{data.esPrice.toFixed(0)}</span>
                  ...
                  <div className="flex justify-between px-2 py-1 rounded bg-gray-800/60">
                    ...
                    <span className={data.esPrice >= data.orb.mid ? 'text-emerald-500' : 'text-red-500'}>
                      {data.esPrice >= data.orb.mid ? 'Above' : 'Below'}
                    </span>
                  </div>
                  {[
                    { label: 'ORB LOW',    price: data.orb.low,  color: 'text-red-400 font-bold', active: data.esPrice <= data.orb.low,  border: true },
                    { label: 'T1 Dn 0.5×', price: data.orb.t1Dn, color: 'text-red-500', active: data.esPrice <= data.orb.t1Dn },
                    { label: 'T2 Dn 1.0×', price: data.orb.t2Dn, color: 'text-red-400', active: data.esPrice <= data.orb.t2Dn },
                    { label: 'T3 Dn 2.0×', price: data.orb.t3Dn, color: 'text-red-300', active: data.esPrice <= data.orb.t3Dn },
                  ].map(row => (
                    <div key={row.label} className={`flex justify-between px-2 py-1 rounded ${row.active ? 'bg-red-950/40' : ''} ${row.border ? 'border-t border-gray-800 pt-2 mt-1' : ''}`}>
                      <span className={row.color}>{row.label}</span>
                      <span className={row.color}>{row.price.toFixed(0)}</span>
                      <span className="text-gray-500">{row.active ? '✓' : `-${(data.esPrice - row.price).toFixed(0)}pt`}</span>
                    </div>
                  ))}
                </div>
                    ...
                    <div className="space-y-1 text-xs mb-3">
                      {[
                        ...
                        <div key={l.label} className="flex justify-between px-2 py-1 rounded bg-gray-800/40">
                          ...
                          <span className="text-gray-500">{Math.abs((l.val as number) - data.esPrice).toFixed(0)}pts</span>
                        </div>
                      ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <DarkCard accent="border-l-blue-700">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="border-t border-gray-800 pt-3">
                    <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">
                      <span className="text-blue-400">⦿ </span>Targets
                    </p>
              ...
              <DarkCard accent="border-l-red-800">
                ...
                <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
                  <div>
                    ...
                    <input type="number" value={accountSize}
                      ...
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-gray-200 text-xs focus:outline-none focus:border-blue-600" />
                  </div>
                  <div>
                ...
                <div className="bg-gray-800 rounded-lg p-3">
                  ...
                  <p className="text-xs text-gray-600 mb-2">{data.contractNote}</p>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {[1, 2, 3, 5].map(contracts => {
                      const riskUSD = data.risk.mesPerStop * contracts;
                      ...
                      return (
                        <div key={contracts} className={`rounded-lg p-2 border ${safe ? 'border-emerald-800 bg-emerald-950/20' : 'border-red-800 bg-red-950/20'}`}>
                          <p className={`font-bold ${safe ? 'text-emerald-400' : 'text-red-400'}`}>{contracts} MES</p>
                    ...
                    {[
                      {
                        ...
                        desc: 'Rising DXY = headwind for ES',
                      ...
                      },
                      {
                        ...
                        desc: 'Rising yields = ES pressure',
                        bearish: (data.internals.tnx?.changePct ?? 0) > 2,
                        bullish: (data.internals.tnx?.changePct ?? 0) < -2,
                      },
                      {
                        label: 'AAPL',
                        data: data.internals.aapl,
                        desc: 'AAPL leads ES / SPX sector',
                        bullish: (data.internals.aapl?.changePct ?? 0) > 1.5,
                        bearish: (data.internals.aapl?.changePct ?? 0) < -1.5,
                      },
                    ].map(item => item.data ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-800 rounded-lg p-2">
                    ...
                    <div className="flex gap-2">
                      <input type="number" value={session.dailyPnL}
                        ...
                        className="flex-1 bg-gray-900 border border-gray-700 rounded-lg px-2 py-1 text-gray-200 text-sm font-bold focus:outline-none focus:border-blue-600" />
                      <span className={`text-sm font-bold self-center ${session.dailyPnL >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        ${(session.dailyPnL * 5).toFixed(0)}
                      </span>
                    </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => saveSession({ ...session, tradesCount: session.tradesCount + 1 })}
                      className="flex-1 py-2 bg-blue-900/40 hover:bg-blue-900/60 border border-blue-800 text-blue-400 text-xs font-semibold rounded-lg">
                      + Log Trade
                    </button>
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-gray-300 text-xs placeholder-gray-600 focus:outline-none focus:border-blue-600 resize-none"
                  />
                </div>
              </div>
            </DarkCard>
