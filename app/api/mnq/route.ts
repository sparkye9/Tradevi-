...
function computeNoTradeFilter(params: {
  ...
  if (params.atr < 8)                                     { reasons.push(`ATR ${params.atr.toFixed(1)} pts — insufficient range (need >8 pts)`); score += 20; }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [nqQ, esQ, vixQ, qqqQ, dxyQ, tnxQ, nvdaQ, candleData] = await Promise.all([
      yQuote('NQ=F'),
      yQuote('ES=F'),
      yQuote('^VIX'),
      yQuote('QQQ'),
    ...
    if (isChop) {
      aiSummary     = `[${sessionCtx}] NQ is in a low-volatility range (ORB: ${orbLow.toFixed(0)}–${orbHigh.toFixed(0)}). ATR is compressed and RSI at ${r} shows no directional edge. Patience is the trade right now.`;
      ...
      aiSummary     = `[${sessionCtx}] NQ (${p}) has broken above the ${tfMin}m ORB high (${orbHigh.toFixed(0)}) and is above VWAP (${v}). RSI ${r} confirms momentum is intact. Conditions favor long continuation.`;
      ...
      aiSummary     = `[${sessionCtx}] NQ (${p}) has broken below the ${tfMin}m ORB low (${orbLow.toFixed(0)}) and is below VWAP (${v}). RSI ${r} confirms bearish momentum. Conditions favor short continuation.`;
      ...
      aiSummary     = `[${sessionCtx}] NQ (${p}) is ${orbStatus === 'inside' ? 'inside the ORB range' : `${orbStatus} the ORB`} with mixed signals. No clear high-probability setup yet. Bias: ${bias}.`;
    ...
    return NextResponse.json({
      ...
      internals: {
        dxy:  dxyQ  ? { price: dxyQ.price,  changePct: dxyQ.changePct,  interpretation: dxyQ.changePct  > 0.3 ? 'Rising DXY → headwind for NQ' : dxyQ.changePct < -0.3 ? 'Falling DXY → tailwind for NQ' : 'DXY neutral' } : null,
        tnx:  tnxQ  ? { price: tnxQ.price,  changePct: tnxQ.changePct,  interpretation: tnxQ.changePct  > 2   ? 'Rising yields → tech pressure' : tnxQ.changePct < -2 ? 'Falling yields → NQ tailwind' : 'Yields stable' } : null,
        nvda: nvdaQ ? { price: nvdaQ.price, changePct: nvdaQ.changePct, interpretation: nvdaQ.changePct > 1.5 ? 'NVDA leading → bullish NQ' : nvdaQ.changePct < -1.5 ? 'NVDA weak → NQ pressure' : 'NVDA neutral' } : null,
      },
