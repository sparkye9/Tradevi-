...
function get8amETTimestampForToday(): number {
  ...
  const now = new Date();
  // 8am ET = (8 - etOffset) UTC hours from midnight UTC
  const utcHour = 8 - etOffset; // e.g. 12 during EDT, 13 during EST
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Fetch window: 2h before 8am ET through now
    ...
    if (text.trimStart().startsWith('<')) throw new Error('Yahoo Finance returned an HTML page — possibly rate-limited');
    ...
    if (candles.length === 0) throw new Error('No valid candles in response — market may be closed or symbol invalid');

    // Find closest candle to 8am ET
