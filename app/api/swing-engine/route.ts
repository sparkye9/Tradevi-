/**
 * Swing Trading & Macro Options Analysis Engine
 * Multi-timeframe structure + institutional liquidity + options positioning
 * Identifies high-probability swing setups with 14–60 DTE contracts.
 */
...
function detectFVGs(candles: CandleData[], symbol: string, tf: FVGLevel['timeframe']): FVGLevel[] {
  ...
  // Return 4 most recent of each type
...
function detectStructure(candles: CandleData[], symbol: string, tf: StructureEvent['timeframe']): StructureEvent[] {
  ...
  for (const sh of recentHighs.reverse()) {
    if (last > sh.price) {
      ...
      events.push({
        symbol, timeframe: tf,
        event: isChoCH ? 'CHoCH_UP' : 'BOS_UP',
        level: r2(sh.price), ageCandles: age,
        significance: age <= 5 ? 'major' : 'minor',
        description: isChoCH
          ? `Bullish CHoCH — broke above ${r2(sh.price)} reversing downtrend`
          : `Bullish BOS — continuation above swing high ${r2(sh.price)}`,
      });
      break;
    }
  }
  ...
  for (const sl of recentLows.reverse()) {
    if (last < sl.price) {
      ...
      events.push({
        symbol, timeframe: tf,
        event: isChoCH ? 'CHoCH_DOWN' : 'BOS_DOWN',
        level: r2(sl.price), ageCandles: age,
        significance: age <= 5 ? 'major' : 'minor',
        description: isChoCH
          ? `Bearish CHoCH — broke below ${r2(sl.price)} reversing uptrend`
          : `Bearish BOS — continuation below swing low ${r2(sl.price)}`,
      });
      break;
    }
  }
...
function classifyRegime(daily: CandleData[], weekly: CandleData[], vix: number) {
  ...
  if (vix > 30) return {
    ...
    description: 'VIX spike — potential capitulation or trend reversal. Market dislocated.',
    tradingApproach: 'Wait for VIX to stabilize. Long strangles for volatility crush plays. Fade extreme moves at HTF support.',
    avoidList: ['Buying expensive premium', 'Naked directional bets', 'Chasing breakdowns'],
  };
  if (atrRatio > 1.3 && dailyB.strength >= 62) return {
    phase: 'expansion' as const,
    description: 'Expansion phase — strong directional momentum, widening daily ranges.',
    tradingApproach: 'Ride momentum. Pullbacks to EMA9/EMA21 are entries. Use trailing stops. Favor calls in uptrend.',
    avoidList: ['Countertrend fades', 'Mean reversion setups', 'Selling naked calls in uptrend'],
  };
  if (vix > 22 && dailyB.strength < 45) return {
    phase: 'distribution' as const,
    description: 'Distribution phase — elevated VIX, weakening breadth, institutional selling.',
    tradingApproach: 'Reduce long exposure. Put debit spreads, protective hedges. Cash is a position.',
    avoidList: ['Aggressive longs', 'Low-delta calls', 'Ignoring stop losses'],
  };
  if (atrRatio < 0.65 && vix < 18) return {
    phase: 'accumulation' as const,
    description: 'Accumulation — low volatility coil. Smart money building positions quietly.',
    tradingApproach: 'Buy dips in strong sectors. Defined risk structures (spreads). Anticipate breakout.',
    avoidList: ['Buying OTM weekly premium', 'Wide stops', 'Overtrading the chop'],
  };
  if (weeklyB?.bias === 'bullish' && dailyB.bias !== 'bearish') return {
    phase: 'expansion' as const,
    description: 'Weekly bullish structure intact. Daily in continuation mode.',
    tradingApproach: 'Trend continuation. Buy pullbacks to EMA21. Calls on dips to HTF FVG.',
    avoidList: ['Shorting into weekly strength', 'Chasing extended breakouts'],
  };
  return {
    phase: 'ranging' as const,
    description: 'Range-bound / developing. No clear directional conviction at current timeframe.',
    tradingApproach: 'Reduce size. Wait for HTF level test or breakout with volume. Iron condors for range-bound.',
    avoidList: ['Large directional bets', 'Momentum chasing', 'Short-DTE options'],
  };
}
...
async function fetchOptionsChain(symbol: string): Promise<{ price: number; calls: YahooOption[]; puts: YahooOption[]; expirationDates: number[] }> {
  try {
    ...
    return {
      price: Number(r.quote?.regularMarketPrice ?? 0),
      calls: (opts.calls ?? []) as YahooOption[],
      puts:  (opts.puts  ?? []) as YahooOption[],
...
export interface ScoredOption {
  ...
  volume: number; openInterest: number;
  iv: number; ivPct: number;
  inTheMoney: boolean; moneyness: number;
  ...
  swingScore: number; grade: 'A+' | 'A' | 'B' | 'C' | 'D';
  rationale: string;
}
...
function scoreContracts(
  contracts: YahooOption[], type: 'call' | 'put',
  symbol: string, currentPrice: number, macroDirection: 'bullish' | 'bearish',
): ScoredOption[] {
  ...
  for (const c of contracts) {
    ...
    // Probability OTM via d2
    const d2 = currentPrice > 0 && c.strike > 0
      ? (Math.log(currentPrice / c.strike) + (-0.5 * iv * iv) * (dte / 365)) / (iv * sqrtT)
      : 0;
    const probabilityOtm = type === 'call' ? r2((1 - ncdf(d2)) * 100) : r2(ncdf(d2) * 100);

    const entryMid = mid;
    const target1  = r2(mid * 1.65);
    const target2  = r2(mid * 2.6);
    const stopLoss = r2(mid * 0.45);
    ...
    // Theta approximation (daily as % of premium)
    const thetaEstDailyPct = dte > 0 ? r2((mid * iv / (2 * sqrtT)) / dte * 100) : 0;

    // Score
    let score = 0;
    score += Math.min(oi / 1000, 1) * 12;           // OI (0-12)
    score += Math.min(volume / 300, 1) * 6;          // Volume (0-6)
    score += Math.max(0, 12 - spreadPct / 3);        // Spread quality (0-12)
    score += dte >= 28 && dte <= 45 ? 14 : dte >= 20 ? 10 : 6;   // DTE sweet spot (0-14)
    score += pctOtm >= 1 && pctOtm <= 7 ? 20 : pctOtm >= 0 && pctOtm < 1 ? 14 : pctOtm > 7 && pctOtm <= 14 ? 10 : 4; // Moneyness (0-20)
    score += ivPct < 35 ? 18 : ivPct < 55 ? 13 : ivPct < 90 ? 8 : 3;   // IV level (0-18)
    score += ((type === 'call' && macroDirection === 'bullish') || (type === 'put' && macroDirection === 'bearish')) ? 18 : 0; // Direction (0-18)
    ...
    out.push({
      contractSymbol: c.contractSymbol, symbol, type,
      strike: c.strike, expiration: c.expiration, dte,
      bid, ask, mid, spread, spreadPct,
      volume, openInterest: oi,
      iv, ivPct, inTheMoney: c.inTheMoney, moneyness,
      deltaApprox, expectedMoveByExp, probabilityOtm,
      entryMid, target1, target2, stopLoss, rrRatio, holdDays,
      thetaEstDailyPct, swingScore, grade,
      rationale: `${dte}DTE · IV ${ivPct}% · OI ${oi.toLocaleString()} · ${spreadPct.toFixed(1)}% spread`,
    });
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Phase 1 — parallel: all quotes + candle sets
    ...
    const sectorRotation = SECTORS.map(s => {
      ...
      return {
        ...
        price:      q?.price     ?? 0,
        changePct1d: q?.changePct ?? 0,
        relStrength: rs,
        trend: (q?.changePct ?? 0) > 0.3 ? 'bullish' as const : (q?.changePct ?? 0) < -0.3 ? 'bearish' as const : 'neutral' as const,
        rank: 0,
      };
    ...
    let allCalls = [...primaryChain.calls];
    let allPuts  = [...primaryChain.puts];

    // Fetch up to 3 more expirations in 14-60 DTE window
    ...
    // Phase 7 — macro outlook
    const vixRegime: 'low' | 'normal' | 'elevated' | 'extreme' =
      vixPrice < 14 ? 'low' : vixPrice < 20 ? 'normal' : vixPrice < 30 ? 'elevated' : 'extreme';
    ...
    const riskEnv: 'risk-on' | 'risk-off' | 'mixed' =
      riskOnSignals >= 2 ? 'risk-on' : riskOffSignals >= 2 ? 'risk-off' : 'mixed';

    const macroTrend = weeklyBias.bias === 'bullish' && dailyBias.bias !== 'bearish' ? 'bullish' as const
      : weeklyBias.bias === 'bearish' && dailyBias.bias !== 'bullish' ? 'bearish' as const : 'neutral' as const;

    const keyRisks: string[] = [];
    if (vixPrice > 25)                keyRisks.push(`VIX ${vixPrice.toFixed(1)} — premium is elevated, consider spreads`);
    if ((tnxQ?.changePct ?? 0) > 3)   keyRisks.push('Yield spike — rate-sensitive sectors under pressure');
    if ((dxyQ?.changePct ?? 0) > 0.5) keyRisks.push('Strong dollar — headwind for risk assets and commodities');
    if (breadth === 'weak')           keyRisks.push('Narrow market breadth — only a few sectors leading');
    if (vixRegime === 'extreme')      keyRisks.push('Extreme volatility regime — avoid premium buying, look for vol crush');
    ...
    const highestConviction = (() => {
      const top = [...scoredCalls, ...scoredPuts].sort((a, b) => b.swingScore - a.swingScore)[0];
      return top ?? null;
    })();
    ...
    return NextResponse.json({
      success: true,
      symbol, currentPrice: r2(currentPrice),

      macroOutlook: {
        trend: macroTrend, riskEnv,
        vix: r2(vixPrice), vixChange: r2(vixQ?.changePct ?? 0), vixRegime,
        ...
        summary: macroTrend === 'bullish'
          ...
          : macroTrend === 'bearish'
          ? `Macro deteriorating — weekly structure bearish, breadth ${breadth}. VIX ${vixPrice.toFixed(1)}, consider defensive positioning.`
          : `Mixed signals: weekly ${weeklyBias.bias}, daily ${dailyBias.bias}. VIX ${vixPrice.toFixed(1)} · ${riskEnv}. Wait for clearer structure.`,
        keyRisks,
      },

      weeklyBias, dailyBias, fourHourBias,
      fvgLevels, structureEvents, marketRegime, sectorRotation,
      scoredCalls, scoredPuts,
      highestConviction, confidenceScore: confidence,

      volatilityData: {
        vix: vixPrice, vixChangePct: r2(vixQ?.changePct ?? 0),
        ivExpanding, regime: vixRegime,
        ...
        recommendation: vixRegime === 'extreme' ? 'Extreme IV — avoid buying premium. Sell strangles or wait for vol crush.'
          : vixRegime === 'elevated' ? 'Elevated IV — prefer debit spreads over naked long options. Wider expected moves.'
          : vixRegime === 'low'      ? 'Low IV — premium is cheap. Excellent environment for buying calls/puts outright. Consider calendars.'
          : 'Normal IV — standard premium buying. Target 30-45 DTE. Defined risk structures preferred.',
      },
    ...
    return NextResponse.json({
      success: false,
      error: err instanceof Error ? err.message : 'Swing engine unavailable',
